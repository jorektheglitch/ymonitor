...
class DBWriter:
    ...
    def coords_event_handler(self, coords:str):
        ...
        if self.coords != coords:
            ...
            secs = int(secs)
            self.cursor.execute(self.write_coords_change, (hour*3600, secs, coords))
            self.conn.commit()
...
class DBReader:
    ...
    def ask_db(self, query, retries=5, delay=0.5)->List[Tuple[str, str]]:
    ...
    def _format_template(self, template:str, end:dt, interval:int):
    ...
    def get_by_hours(self, interval:str):
